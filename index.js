...
function compareDates(date1, date2) {
  ...
  if (typeof date1.getTime && typeof date2.getTime !== "function") {
    // Якщо date не є об'єктом Date, повертаємо рядок
    return "Помилка: вхідне значення має бути об'єктом Date";
  }
  ...
  const formatIso1 = date1.toISOString();
  const formatIso2 = date2.toISOString();
...
function getDaysDifference(startDate, endDate) {
  // Перевірка, чи є вхідні значення об'єктами Date.
  if (typeof startDate.getTime && typeof endDate.getTime !== "function") {
    // Якщо date не є об'єктом Date, повертаємо рядок
    return "Помилка: вхідне значення має бути об'єктом Date";
  }
  ...
  // Збереження дат для виведення в форматі ISO.
  const formattedIsoStart = startDate.toISOString();
  const formattedIsoEnd = endDate.toISOString();

  // Повертаємо об'єкт з початковою та кінцевою датами та різницею в днях.
  return {
    startDate: formattedIsoStart,
    endDate: formattedIsoEnd,
